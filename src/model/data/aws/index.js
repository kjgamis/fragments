...
async function deleteFragment(ownerId, id) {
  ...
  const commandS3 = new DeleteObjectCommand(s3Params);
  const commandDynamoDB = new DeleteCommand(dynamoParams);

  try {
    await s3Client.send(commandS3);
    await ddbDocClient.send(commandDynamoDB);
  } catch (err) {
    logger.error({ err, s3Params, dynamoParams }, 'Error deleting fragment data from S3 and DynamoDB');
